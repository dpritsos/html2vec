...
class Html2TF(BaseHtml2TF):
    ...
    def yield_(self, xhtml_file_l, tid_vocabulary, norm_func, encoding, error_handling):
        ...
        for html_str in self.load_files(xhtml_file_l, encoding, error_handling):
            fq_lst.append( self.tl2tf.trms2f_sparse( Html2TF.s2ngl.terms_lst( self.html_attrib( html_str ) ), tid_vocabulary, norm_func) )

        #Pack it as a sparse vstack and return it
        copus_fq_array = ssp.vstack( fq_lst )
        return ( ssp.csr_matrix(copus_fq_array, shape=copus_fq_array.shape, dtype=np.float32), tid_vocabulary )


    def from_src(self, xhtml_str):
        raise Exception("Please use from_files() or from_paths() methods instead")

    def from_files(self, xhtml_file_l, tid_vocabulary=None, norm_func=None, encoding='utf8', error_handling='strict'):
        return self.yield_(xhtml_file_l, tid_vocabulary, norm_func, encoding, error_handling)  


    def from_paths(self, basepath, filepath_l, tid_vocabulary=None, norm_func=None, encoding='utf8', error_handling='strict'):
        ...
        matrix, tid_dict = self.from_files(xhtml_file_l, tid_vocabulary, norm_func, encoding, error_handling)
